// deno-lint-ignore-file no-case-declarations no-explicit-any no-unused-vars
import { assert } from './assert.ts';
import { ErrorHandler } from './error-handler.ts';
import { Messages } from './messages.ts';
import * as Node from './nodes.ts';
import { Comment, RawToken, Scanner, SourceLocation } from './scanner.ts';
import { Syntax } from './syntax.ts';
import { Token, TokenName } from './token.ts';
...
export class Parser {
    ...
    throwError(messageFormat: string, ...values: Array<any>): void {
    ...
    tolerateError(messageFormat: string, ...values: Array<any>) {
        const args = values.slice();
        const msg = messageFormat.replace(/%(\d)/g, (whole, idx) => {
            assert(idx < args.length, 'Message reference must be in range');
            return args[idx];
    ...
    throwUnexpectedToken(token?: any, message?: string): never {
        throw this.unexpectedTokenError(token, message);
    }

    tolerateUnexpectedToken(token?: any, message?: string) {
    ...
    collectComments() {
        if (!this.config.comment) {
            ...
            const comments = this.scanner.scanComments() as Comment[];
    ...
    startNode(token: any, lastLineStart = 0): Marker {
    ...
    finalize(marker: Marker, node: any) {
        if (this.config.range) {
    ...
    expectKeyword(keyword: string | number) {
        const token = this.nextToken();
    ...
    matchContextualKeyword(keyword: string | number) {
    ...
    isolateCoverGrammar(parseFunction: any) {
    ...
    inheritCoverGrammar(parseFunction: any) {
        const previousIsBindingElement = this.context.isBindingElement;
        const previousIsAssignmentTarget = this.context.isAssignmentTarget;
        const previousFirstCoverInitializedNameError = this.context.firstCoverInitializedNameError;
    ...
    parsePrimaryExpression(): Node.Expression {
        ...
        switch (this.lookahead.type) {
            case Token.Identifier:
                ...
                expr = this.matchAsyncFunction() ? this.parseFunctionExpression() : this.finalize(node, new Node.Identifier(this.nextToken().value as string));
                break;
    ...
    parseObjectPropertyKey(): Node.PropertyKey {
        ...
        switch (token.type) {
            ...
            case Token.NullLiteral:
            case Token.Keyword:
                key = this.finalize(node, new Node.Identifier(token.value as string));
                break;
    ...
    isPropertyKey(key: any, value: any) {
    ...
    parseObjectProperty(hasProto: any): Node.Property {
        ...
        if (token.type === Token.Identifier) {
            ...
            key = isAsync ? this.parseObjectPropertyKey() : this.finalize(node, new Node.Identifier(id as string));
        } else if (this.match('*')) {
            this.nextToken();
        } else {
            computed = this.match('[');
        ...
        if (token.type === Token.Identifier && !isAsync && token.value === 'get' && lookaheadPropertyKey) {
            ...
            if (this.match(':') && !isAsync) {
                ...
                const id = this.finalize(node, new Node.Identifier(token.value as string));
                if (this.match('=')) {
    ...
    reinterpretExpressionAsPattern(expr: any) {
        switch (expr.type) {
        ...
        if (this.match('++') || this.match('--')) {
            ...
            if (!this.hasLineTerminator && this.lookahead.type === Token.Punctuator) {
                if (this.match('++') || this.match('--')) {
                    ...
                    const prefix = false;
                    expr = this.finalize(this.startNode(startToken), new Node.UpdateExpression(operator as string, expr, prefix));
                }
            }
        }
    ...
    parseBinaryExpression(): Node.Expression {
        ...
        const updateNullishCoalescingRestrictions = (token: any): void => {
    ...
    checkPatternParam(options: any, param: any) {
        switch (param.type) {
            case Syntax.Identifier:
    ...
    parseAssignmentExpression(): Node.Expression {
        ...
        if (!this.context.allowYield && this.matchKeyword('yield')) {
            ...
            if (expr.type === ArrowParameterPlaceHolder || this.match('=>')) {
                ...
                this.context.isAssignmentTarget = false;
                this.context.isBindingElement = false;
                const isAsync = (expr as any).async;
                ...
                if (this.matchAssign()) {
                    ...
                    expr = this.finalize(this.startNode(startToken), new Node.AssignmentExpression(operator, expr as Node.Expression, right));
                    this.context.firstCoverInitializedNameError = null;
                }
            }
        }
    ...
    parsePropertyPattern(params: any, kind?: string): Node.Property {
        ...
        if (this.lookahead.type === Token.Identifier) {
            ...
            const init = this.finalize(node, new Node.Identifier(keyToken.value as string));
            if (this.match('=')) {
    ...
    parseRestProperty(params: any): Node.RestElement {
        const node = this.createNode();
        this.expect('...');
    ...
    parseForStatement(): Node.ForStatement | Node.ForInStatement | Node.ForOfStatement {
        ...
        return (typeof left === 'undefined') ?
            ...
            forIn ? this.finalize(node, new Node.ForInStatement(left, right as Node.Expression, body)) :
                this.finalize(node, new Node.ForOfStatement(left, right as Node.Expression, body, _await));
    }
    ...
    parseSwitchCase(): Node.SwitchCase {
        ...
        return this.finalize(node, new Node.SwitchCase(test as Node.Expression, consequent));
    }
    ...
    validateParam(options: any, param: any, name: string) {
        const key = '$' + name;
        if (this.context.strict) {
    ...
    parseFormalParameter(options: any) {
        const params: any[] = [];
    ...
    parseExportDeclaration(): Node.ExportDeclaration {
        ...
        if (this.matchKeyword('default')) {
            ...
            exportDeclaration = this.finalize(node, new Node.ExportNamedDeclaration(declaration as any, [], null));
